...
var Game = {
	...
	create: function () {
		...
		for (var i = 1; i < initialTailLength + 1; i++) {
			tail[i - 1] = game.add.sprite(x - (i * gridSize) + (borderSide + 2), y + 1, 'head');
		...
		game.time.events.add(Phaser.Timer.SECOND * 0,startGame, this)

		function startGame(){
		if (speed == 10 && gamestart == "intro"){
			this.getready.destroy();
			gamestart = "running"
				
		}else{
		this.getready = this.game.add.text(400, 300, 'Get Ready!', { font: "1.84rem Wendy One", fill: textColour });
		this.getready.anchor.setTo(1.85, 4)	
		gamestart = 'intro'
		    
			game.time.events.add(Phaser.Timer.SECOND * 1.5,startGame, this)
	...
	_trigger_death: function () {
		this.yourhighscore = this.add.text(400, 300, 'Your HighScore:' +`${highscore}`, { font: "1.84rem Wendy One", fill: textColour });
		this.yourhighscore.anchor.setTo(1.35, 6.2)
		this.tryagaintext = this.add.text(400, 300, 'Click Here Try Again', { font: "1.84rem Wendy One", fill: textColour });
		this.tryagaintext.anchor.setTo(1.21, 4.2)
		this.tryagaintext.inputEnabled = true;
		this.tryagaintext.events.onInputDown.add(this.restartGame,this);
		this.gameovertext = this.add.text(400, 300, 'Game Over', { font: "2rem Wendy One", fill: textColour });
		this.gameovertext.anchor.setTo(1.75, 3)
